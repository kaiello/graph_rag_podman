#!/usr/bin/env python3
"""
Integrated Docling Extractor v6.6 (Graph RAG Schema - Lumped Strategy)
Features:
...
7. RAG Chunking: Generates JSONL with 'kb_Content_Block' schema and embeddings.
   * UPDATE v6.6: Uses 'Lumped' strategy (max 4000 chars) to keep pages intact.
"""
...
class ChunkProcessor:
    def __init__(self, chunk_size=4000, chunk_overlap=200):
        ...
        This is intentionally large to keep full pages as single 'lumped' blocks,
        matching the target example format.
        """
        # NOTE: Removed MarkdownHeaderTextSplitter to prevent fragmentation of headers.
        
        # Split recursively to respect token limits only if page is massive
    ...
    def create_chunks_for_page(self, markdown_text: str, page_number: int, base_filename: str, s3_uri: str, start_index: int, orig_elements: str = None) -> List[dict]:
        ...
        # Feed the entire page text directly to the splitter.
        # It will likely return a list with 1 item (the whole page) unless > 4000 chars.
        final_docs = self.text_splitter.create_documents([markdown_text])
...
def process_document_chunks(doc: DoclingDocument, output_dir: Path, base_name: str, s3_uri: str):
    ...
    # Initialize processor with large chunk size
    processor = ChunkProcessor(chunk_size=4000, chunk_overlap=200)
    ...
    # Save as JSONL
    jsonl_path = chunks_dir / f"{base_name}_embeddings.jsonl"
